...
import { 
  Zap, FileBarChart, RefreshCw, AlertTriangle, TrendingUp, 
...
export const AnalyticsView = ({ products, history, exchangeRate, sites }: AnalyticsViewProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const r = await getAutomatedReport(products, history, exchangeRate, sites);
        setReport(r);
        const analysis = await getAutomatedAnalysis(products, history);
        setAutomatedAnalysis(analysis);
      } catch (err) {
    ...
    fetchData();
  }, [products, history, exchangeRate, sites]);
  ...
  return (
    <div className="space-y-12 animate-fade-in pb-32">
      ...
      {/* SECTION HAUTE : BILAN & AUTOMATISATION */}
              ...
              <ResponsiveContainer width="100%" height="100%">
                 <BarChart data={report?.topConsumption}>
                    ...
                    <Bar dataKey="valeur" fill="#f97316" radius={[12, 12, 0, 0]}>
                       {report?.topConsumption.map((entry, index) => (
              ...
              <ResponsiveContainer width="100%" height="100%">
                 <PieChart>
                    <Pie
                       ...
                       label={({name}) => name}
                    >
                       {report?.siteExpenseData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} />
                       ))}
                    </Pie>
                    <Tooltip contentStyle={{borderRadius: '20px'}} />
