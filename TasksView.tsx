  ...
  return (
    <div className="space-y-10 animate-fade-in pb-32">
       <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
          <h3 className="text-xl font-header italic uppercase mb-8 flex items-center gap-3">
             <Plus className="w-5 h-5 text-emerald-500" /> Planifier une nouvelle tâche
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
             <div className="lg:col-span-2 space-y-2">
                ...
                <input 
                   ...
                   className="w-full bg-slate-50 border border-slate-100 p-4 rounded-2xl text-[11px] font-black uppercase italic outline-none focus:ring-2 focus:ring-[#1a3a22]"
                />
             </div>
       ...
       <div className="grid grid-cols-1 gap-4">
          <div className="flex items-center gap-4 mb-2">
             <h3 className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-6">Liste des engagements</h3>
             <div className="flex-1 h-px bg-slate-100"></div>
          </div>
          {tasks.length === 0 ? (
            <div className="text-center py-20 opacity-20">
               <ListTodo className="w-20 h-20 mx-auto mb-4" />
               <p className="text-[12px] font-black uppercase italic">Aucune tâche planifiée</p>
            </div>
          ) : (
            tasks.map((task) => {
              ...
              return (
                <div 
                  key={task.id} 
                  className={`bg-white p-6 rounded-[2.5rem] border transition-all flex items-center justify-between group shadow-sm ${
                ...
                >
                   <div className="flex items-center gap-6">
                      ...
                      <div>
                         <div className="flex items-center gap-3">
                            ...
                            <Badge variant={task.priority === 'Haute' ? 'danger' : task.priority === 'Moyenne' ? 'warning' : 'success'}>{task.priority}</Badge>
                            {today && task.status === 'En attente' && <Badge variant="info">AUJOURD'HUI</Badge>}
                            {overdue && <Badge variant="danger">EN RETARD</Badge>}
                         </div>
                         <div className="flex items-center gap-5 mt-2 text-slate-400 text-[10px] font-bold uppercase">
                            <div className="flex items-center gap-2">
                               <Calendar className={`w-3.5 h-3.5 ${today ? 'text-emerald-500' : 'text-slate-300'}`} /> 
                               <span className={today ? 'text-slate-900' : ''}>{task.day} {new Date(task.date).toLocaleDateString()}</span>
                            </div>
                            <div className="flex items-center gap-2">
                               <Clock className={`w-3.5 h-3.5 ${today ? 'text-emerald-500' : 'text-slate-300'}`} /> 
                               <span className={today ? 'text-slate-900' : ''}>{task.time}</span>
                            </div>
                            {task.status === 'En attente' && (
                              <div className="flex items-center gap-2 ml-4">
                                <BellRing className={`w-3.5 h-3.5 ${overdue ? 'text-rose-500 animate-pulse' : 'text-slate-200'}`} />
                                <span className="text-[8px] tracking-widest">RAPPEL ACTIF</span>
                              </div>
                            )}
                         </div>
                      </div>
                   ...
                   <div className="flex items-center gap-2">
                      <button onClick={() => deleteTask(task.id)} className="p-3 bg-rose-50 text-rose-300 hover:text-rose-500 hover:bg-rose-100 rounded-2xl opacity-0 group-hover:opacity-100 transition-all shadow-sm">
