...
import { 
  ...
  Package, Lamp, ShieldCheck, ArrowRightLeft, Search, Box, CheckSquare
} from 'lucide-react';
    ...
    <div className="space-y-10 animate-fade-in pb-32">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {sites.map(site => {
          ...
          return (
            <div key={site.id} className="bg-white rounded-[4rem] border border-slate-100 shadow-xl overflow-hidden group hover:border-emerald-200 transition-all flex flex-col">
          ...
          <form onSubmit={handleSubmit} className="bg-white w-full max-w-xl rounded-[3.5rem] shadow-2xl p-12 space-y-8 animate-slide-in">
            ...
            <div className="space-y-6">
               <div className="space-y-2">
                  ...
                  <input required type="text" placeholder="DEPOT CENTRAL SUD" value={newSiteData.name} onChange={(e) => setNewSiteData({...newSiteData, name: e.target.value})} className="w-full bg-slate-50 border border-slate-100 p-5 rounded-2xl text-[12px] font-black uppercase outline-none focus:ring-2 focus:ring-emerald-500" />
               </div>
